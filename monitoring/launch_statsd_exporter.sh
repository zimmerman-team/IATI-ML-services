...
screen -L -Logfile ${LEARNING_SETS_DIR}/logs/statsd_exporter_${TS}.log \
  -S statsd_exporter -d \
  -m ${LEARNING_SETS_DIR}/monitoring/statsd_exporter/statsd_exporter \
  --web.listen-address="127.0.0.1:9102" \
  --statsd.listen-udp="127.0.0.1:9125" \
  --statsd.listen-tcp="127.0.0.1:9125" \
  --statsd.mapping-config=${LEARNING_SETS_DIR}/monitoring/statsd_exporter_mapping.conf
